...
def main():
    ...
    try:
        ...
        # Otherwise, if no rules are violated, set the label to Safe (overriding model prediction).
        if is_unsafe_by_rules:
             label = "Unsafe"
        else: # This condition implies not is_unsafe_by_rules, i.e., all rules are within limits
             label = "Safe"
